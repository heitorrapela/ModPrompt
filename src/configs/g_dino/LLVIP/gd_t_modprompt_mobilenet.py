...
model = dict(
    ...
    fft = 0, hft = 0, img_prompt='modprompt',
...
test_pipeline = [
    dict(type='LoadImageFromFile', backend_args=_base_.backend_args),
    dict(type='FixScaleResize', scale=(1280, 1024), keep_ratio=True),
    dict(type='LoadAnnotations', with_bbox=True),
    dict(
        type='PackDetInputs',
        meta_keys=('img_id', 'img_path', 'ori_shape', 'img_shape',
...
train_dataloader = dict(
    ...
    dataset=dict(
        data_root=data_root,
        ann_file='Annotations/llvip_ir_fulldataset_train.json',
        metainfo=metainfo,
...
val_dataloader = dict(
    ...
    dataset=dict(data_root=data_root,
        ann_file='Annotations/llvip_ir_test.json',
        metainfo=metainfo,
        pipeline=test_pipeline, return_classes=True))
test_dataloader = val_dataloader

val_evaluator = dict(ann_file='data/LLVIP/infrared/Annotations/llvip_ir_test.json')
