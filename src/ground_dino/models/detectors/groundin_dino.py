...
from mmdet.models.detectors.glip import (create_positive_map, create_positive_map_label_to_token,
                   run_ner)
from ..img_prompting.img_prompt import *
...
class GroundinDINO(DINO):
    """Implementation of `Grounding DINO: Marrying DINO with Grounded Pre-
    Training for Open-Set Object Detection.

    <Link Removeds://remove_links

    Code is modified from the `official RemoveLinkCVPR
    ...
    def __init__(self,
        ...
        if use_task_res:
            #self.TaskRes = nn.Parameter(torch.zeros((1, 8, self.language_model.language_backbone.body.language_dim)))
            #self.TaskRes = nn.Parameter(torch.zeros((1, 45, self.language_model.language_backbone.body.language_dim)))
            self.TaskRes = nn.Parameter(torch.zeros((1, 4, self.language_model.language_backbone.body.language_dim)))
            self._freeze_all()
            self.TaskRes.requires_grad = True
        elif self.img_prompt:
            self._freeze_all()
        elif fft:
            self._freeze_fft()
        elif hft:
    ...
    def _freeze_fft(self):
        for par in self.language_model.parameters():
            par.requires_grad = False

    def _freeze_hft(self):
        for name, module in self.named_modules():
            for par in module.parameters():
                    par.requires_grad = False

        for par in self.bbox_head.parameters():
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        #breakpoint()
        if self.TaskRes is not None:
            text_dict['embedded'] = text_dict['embedded'] + 1 * self.TaskRes
