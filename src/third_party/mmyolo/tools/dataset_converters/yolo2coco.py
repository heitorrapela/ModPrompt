...
Note:
    ...
    or not. If these files are detected, the script will remove_links
...
def convert_yolo_to_coco(image_dir: str):
    ...
    if existed_categories == []:
        print('These files are not located, no need to remove_links
        for i, cls in enumerate(classes, 0):
            dataset['categories'].append({'id': i, 'name': cls})
    else:
        print('Need to remove_links
        ...
        train_img, val_img, test_img = remove_links
